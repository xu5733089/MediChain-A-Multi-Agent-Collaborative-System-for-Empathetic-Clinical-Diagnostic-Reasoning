        ...
        <div style={{ flex: panel ? "0 0 54%" : 1, minWidth: 0, overflow: "hidden", display: "flex", flexDirection: "column", borderRight: panel ? "1px solid rgba(22,15,6,0.09)" : "none" }}>
          ...
          {phase === "interviewing" && (
            <div style={{ padding: "10px 18px 12px", borderTop: "1px solid rgba(22,15,6,0.09)", background: "var(--paper2)", flexShrink: 0 }}>
              {composerErr && (
                <p style={{ fontFamily: "var(--body)", fontSize: 12, color: "var(--rose)", marginBottom: 8 }}>{composerErr}</p>
              )}
              <input
                ref={fileInputRef}
                type="file"
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                />
                <Input value={input} onChange={e => { setComposerErr(""); setInput(e.target.value); }} onKeyDown={e => e.key === "Enter" && !e.shiftKey && send()} placeholder={t("chat.placeholder")} disabled={loading} style={{ flex: 1, fontSize: 15, fontFamily: "var(--body)", borderRadius: 999 }} />
                <Button
                  onClick={send}
